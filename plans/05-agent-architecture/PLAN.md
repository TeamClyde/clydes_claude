...
| "What does this function actually do?" (needs code comprehension) | No — read the file in main context | — |
...
**Output:**
A direct answer — an ARN, a parameter value, a resource name — not a dump of raw API response. If the value cannot be found, say so explicitly rather than speculating.
...
**What it does NOT do:**
- Synthesize or reason about what it finds
...
| **researcher scope** | Narrowed to live MCP lookups only (ARNs, SSM parameters, infrastructure values). Codebase lookups (symbol location, callers, return types) belong in the main context using Grep+Read directly — spawning an agent for retrieval that completes in under a second adds overhead without benefit. Parallel dispatch remains valid when multiple independent MCP queries need to run simultaneously. |
