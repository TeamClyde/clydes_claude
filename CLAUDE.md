...
Skip for S-sized mechanical tasks (renaming, config-only, single-line fixes). The architect loops until blockers clear; after 3 rounds with `error`-severity findings still remaining, it pauses and surfaces them to the user as a checkpoint (continue / intervene / accept) rather than silently stopping or attempting a fourth round unbidden.
