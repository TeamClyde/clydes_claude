...
graph TB
  ...
  subgraph SEED["Seed / reference"]
    ...
    GENERATED["Generated artifacts: component-inventory.md (79 components), gate-map.md (232 edges)"]
  end
...
- `docs/reference/component-inventory.md` — the complete list of all 79 components (42 skills, 13 agents, 14 rules, 10 hooks) with type, name, model/event, and description. This is the authoritative "what exists" record — narrative docs cite it rather than duplicating lists that would drift. An invariant in `scripts/harvest-components.test.mjs` fails if any committed component candidate produces no node, so the list cannot lose an entry silently.
- `docs/reference/gate-map.md` — 232 directed edges representing explicit references between components (which skill invokes which agent, which rule constrains which skill, etc.). **This is a checked contract, not merely a generated picture.** It is regenerated on every run and byte-diffed by `npm run harvest:check`, and both halves are enforced by blocking tests in CI: the edge set is derived from the shared citation tokenizer (`scripts/lib/component-refs.mjs`), and `scripts/reference-integrity.test.mjs` fails the build when a citation anywhere in the committed corpus resolves to nothing. A reference to a component that does not exist can no longer pass unnoticed.
