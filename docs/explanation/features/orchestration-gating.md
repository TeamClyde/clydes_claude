...
**Escalation reasons are not interchangeable.** `uncertain` and `disagree` say the claim may be *wrong*; `unsupported` says the premise is not supported (dropped at Tier 1 under most profiles, though `web-research` escalates it instead per its `guard-unsupported` bias); `thin-source` says the claim may be *right but under-evidenced*, and escalates a finding Tier 1 would otherwise have passed through as `supported`. Thinness is a judgement about the source rather than a verdict against the claim, so it is never itself a drop reason — it buys more scrutiny, and the flag reaches the reader's evidence table. It does not make the finding drop-proof: an escalated thin finding faces the same Tiers 2 and 3 as any other and can still be dropped there on the merits of the claim. Routing thin findings toward *better* evidence is **not implemented in the verify engine** — Tier 2 re-reads the existing source, never a new one, and verify's own escalation stays uniformly subtractive (every tier can only keep or drop). The `librarian` consumer builds that remediative stage **downstream of verify**, on the `web-research` profile only: it diagnoses why the evidence is thin, applies one matching query shift, re-researches, and re-verifies against the original sub-question before merging candidates that must clear `supported && !thinSource` — never dropping the thin findings that triggered it ([ADR-0012](../adr/0012-remediative-escalation-for-thin-source.md)). See `skills/dispatching-parallel-agents/references/verify-protocol.md` § What each escalation reason means.
...
### Synthesis repair and verify's excerpt-first escape lane

**`checkRunHealth` (`scripts/lib/run-health.mjs`) is the consumer of engine signals that were already being computed and discarded.** Every real incident this run-health module was built to catch had already been caught by a human reading a failed run — in each case the signal that would have caught it automatically was already returned by `parallelFanout` or `tieredVerify`, rendered as a prose note a reader skims past. `checkRunHealth` runs at the end of both `.workflow.mjs` scripts (`librarian.workflow.mjs` and `orchestration-audit.workflow.mjs`) and turns those signals into hard failures: a model-pin assertion when more than 10 units were dispatched with `modelTier: null`, a hard-fail when verify reports `verifyEmptied` or any coverage fraction (`triageCoverage` / `recheckCoverage` / `consensusCoverage`) below 1, and a stitch-completeness check — every `sections[i].subQuestion` the run intended to publish must appear as a substring of the assembled `report`, catching silent truncation. `ok: false` means REPORT UNHEALTHY, not DISCARD — the caller still hands over its artifacts; dropping paid-for work is not this module's job. A related but separate PURE helper, `watchdogMargin` (`scripts/measure-workflow-run.mjs`), flags an agent that lands at or above 70% of its phase's `timeoutMs`; it lives outside `checkRunHealth` because duration measurement needs timers the Workflow sandbox does not permit (`Date.now()`/`Math.random()`/`new Date()` all throw there), so it runs offline against transcripts instead.

**The excise repair loop replaces full-section regeneration.** When the L2 traceability audit flags a section, the pipeline no longer regenerates the whole section and re-audits it from scratch — it dispatches a cheap excise agent that deletes or softens ONLY the flagged claims. `exciseGuard` (`scripts/lib/librarian-core.mjs`, function `exciseGuard`) enforces two deterministic guards on the excise output, checked in a fixed order (length, then URL membership): the output must not be longer than its input, and it must not introduce any URL absent from the section's own findings (reusing the same set-membership check L1 already applies to the writer's prose). If either guard fails, the pipeline falls back to publishing the section as-is and records the flagged claims in the run's `integrity` channel — paid-for work is never silently discarded, the same preservation principle `excerptGuard` follows above.

**Verify's Tier-2 excerpt-first path, with a live escape lane (`scripts/lib/verify.mjs`).** On the `'web-research'` profile only, Tier 2 (`clusteredRecheck`) first judges a finding against its carried `excerpt` instead of re-fetching the source — the research agent already fetched and read the page, and re-fetching it had been the dominant cache-read cost of a librarian run. It escalates to a live re-fetch (the original, pre-existing behavior) for any finding where: the recheck agent sets `needsSource: true` on its verdict, the excerpt is missing or shorter than `MIN_EXCERPT_CHARS` (80 chars — the predicate is `needsLiveRecheck` in `scripts/lib/librarian-core.mjs`, mirrored inline in `verify.mjs` as `needsLive` since `verify.mjs` is dependency-free by design), or the finding is `contested`. Tier 3 (`consensus`) always live-fetches regardless of profile — it is never excerpt-only, so ground truth stays exactly one hop away for the escalation path. `scripts/orchestration-audit.workflow.mjs`, the other consumer of the same verify code, is unaffected: it runs the `'audit'` profile, which forces `excerptFirst` to false, so its recheck behavior is byte-identical to before this change.

```mermaid
flowchart LR
  T2["Tier 2 recheck<br/>judges finding against excerpt"] --> D{"escalate?"}
  D -->|"needsSource:true"| LIVE["live re-fetch<br/>(original source read)"]
  D -->|"excerpt missing / < 80 chars"| LIVE
  D -->|"contested"| LIVE
  D -->|"none of the above"| KEEP["excerpt verdict stands"]
  LIVE --> T3["Tier 3 consensus<br/>always live-fetches"]
  KEEP --> T3
```
...
- [ADR-0012](../adr/0012-remediative-escalation-for-thin-source.md) — Remediative escalation for `thin-source`, in the consumer rather than the verify engine (Accepted)
