...
**Escalation is not uniformly subtractive.** Every `escalateOn` reason routes a finding toward a tier that can only drop it — except one. The `web-research` profile's `thin-source` reason is **remediative**: `uncertain` and `disagree` mean the claim may be *wrong* (drop it if it cannot survive adversarial challenge), but `thin-source` means the claim may be *right but under-evidenced* (route it toward better evidence, not toward deletion). Deleting a working pointer to a weak source loses the pointer without gaining accuracy, so thin findings are never dropped for being thin — only `web-research` uses this remediative path, and only after Tier 1 has flagged the finding. See `skills/dispatching-parallel-agents/references/verify-protocol.md` § Subtractive vs remediative escalation for the full rationale.
