...
- `rules/source-text-assertions.md` governs any test that asserts against a file's own source text rather than importing it — the shape forced on the Workflow bundles, which read top-level globals and end in a top-level `return`, so `import()` throws. It defines the guarded-`indexOf` idiom: bound every region at both ends and assert both markers resolve before slicing. Consumed by `agents/test-builder.md` when it writes such a test, and enforced in-repo by a meta-test in `scripts/librarian.workflow.test.mjs` that fails on any inline `slice(x.indexOf(...))`. The failure mode it exists to prevent is a false PASS, not a failure: `slice(start, -1)` means "to the second-to-last character", and `-1 < anything` is true, so a vacuous assertion reports green while testing nothing.
