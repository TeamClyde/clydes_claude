...
**`docs-refresh`** is a thin router that dispatches on-demand content generation to the right specialist. The `feature` and `architecture` routes go through `doc-author`; the `adr` route goes through `architecture-decision-records`; `changelog` and `openapi` go through their respective skills (the wshobson `api-documenter` agent backs API-reference authoring); `tutorial`, `how-to`, `reference`, and `diagram` route to wshobson agents (`tutorial-engineer`, `docs-architect`, `reference-builder`, `mermaid-expert`).
