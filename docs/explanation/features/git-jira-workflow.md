...
- **`plan-management` skill** — sibling skill that owns the three-write atomic operation (journal append + plan section edit + handoff refresh) when a divergence occurs. `git-manager` does not write to the journal — that invariant belongs to `plan-management`'s `divergence` mode.
