...
- **Tiered-adversarial verify is a sampling pass, not a proof.** A real finding can still slip through. The blind-canary recall harness (`scripts/recall/`) is the false-negative smoke signal. Fallback is per-tier and scoped to the unit that failed — a lost cluster or chunk, never the whole run — so on a tier failure or timeout the result never silently hangs or silently loses findings. Only a whole-Tier-1 or whole-Tier-3 collapse actually degrades the verify: it stamps `degraded: true` and `degradedAtTier` (`'triage'` | `'consensus'` | `null` — no `'clusteredRecheck'` value exists). A whole-Tier-2 collapse is contained, not degraded — it reports `recheckCoverage: 0` with `degraded` staying `false`, because every escalated finding still goes on to face Tier 3. A whole-tier collapse never unwinds the tiers that already completed: a lost Tier 3 still returns the Tier-1 `supported` set plus the contested tail.
...
**`degraded` / `partial` / `degradedAtTier`** — Three distinct signals `tieredVerify` returns, and callers must not conflate them. `degraded: true` means the verify did not run for the findings it covers — discard the output, treat those findings as unverified. `partial: true` means the verify ran and its output is usable, but some tier agents were lost, so it was less adversarial than the protocol promises — read `recheckCoverage` / `consensusCoverage` for the extent. `degradedAtTier` is the **cause**, not a status: `'triage'` | `'consensus'` | `null`, naming which tier collapsed wholesale (or `null` on a clean run). There is no `'clusteredRecheck'` value — a whole-Tier-2 collapse is contained (each cluster falls back to its own members, which still face Tier 3) rather than degrading, so it surfaces only as `partial: true` with `recheckCoverage: 0`.
