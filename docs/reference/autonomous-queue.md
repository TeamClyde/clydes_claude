...
| q002 | 113, 151 | 3 | `fix/verify-chunking` | done | 0 | 0 | `scripts/lib/verify.mjs`, `scripts/lib/verify.test.mjs` | `npm test` → 0 | yes |
...
- **q002** — #113 and #151 were defects *in* `fix/verify-chunking` (base was that branch,
  since on `main` the code did not yet exist). Shipped in PR #166 (commit `ff0537a`) on
  2026-08-05. Five issues touch `verify.mjs`, so these were serialized as one solo row.
