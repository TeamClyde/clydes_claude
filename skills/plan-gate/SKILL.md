...
**Failure mode — mid-gate failure:** If plan-gate does not reach Step 6 — either an earlier step is ABANDONED, or the architect gate hits its 3-round pause and the user does NOT choose `accept` (they keep resolving/intervening, or abandon the gate) — the `:divergence` call is not invoked, and the plan doc gate-checkbox section and journal remain in their pre-gate state. (If the user chooses `accept` at the 3-round pause, the gate proceeds to Step 6 normally.) Resuming or re-running plan-gate from the beginning is fully idempotent — when it eventually succeeds it fires `:divergence` once with the final stage outcomes.
