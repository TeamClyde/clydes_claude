...
name as BLOCKING with source file, reference text, and what's missing.
```
...
name differs by casing. Report mismatches as BLOCKING.
```
...
is between a rule and a skill (rule wins by priority), report as BLOCKING instead.
```
...
risk) unless the conflict would cause silent incorrect behavior, in which case BLOCKING.
```
...
INFO. Note: some skills are intentionally standalone (e.g., pulser) — flag only if
...
Report clear mismatches (trigger fires in the wrong context) as BLOCKING.
```
...
inventory — flag components in the map that no longer exist (BLOCKING) and components that exist
but aren't in the map (INFO). Report missing handlers as BLOCKING, stale map entries as
WARNING, unlisted-but-existing components as INFO.
```
