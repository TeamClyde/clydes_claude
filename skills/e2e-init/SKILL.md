...
Emit `scripts/run-tests.sh` using the **Firmware static-analysis backbone**
template in `references/run-tests-templates.md` — write its content verbatim,
then adapt the `# REPLACE:` placeholders with any
...
Emit `scripts/run-tests.sh` using the **Non-Firmware host-runner** template in
`references/run-tests-templates.md` — write its content verbatim, then replace
the placeholder comments with the actual service detection blocks and run
commands derived from Step 4 and the frameworks identified in Step 3.
