...
Invoke the architect **4-lens finding-space panel** in `plan` mode before ExitPlanMode (Shape A — Dimensional-review panel, per `dispatching-parallel-agents` §"Dispatching in prose"). The gate sequence is:
...
Dispatch one `subagent_type: architect` agent **per lens**, all in parallel. The 4 lenses come from `agents/architect.md` § Review Lenses: L1 — correctness & coherence; L2 — grounding & self-containment (owns both sweeps — Symbol-Verification & Callers, and Framework & External-Behavior Assumption — run ONCE under L2); L3 — systemic & standards (folds stack-hat adherence + cost/scale/blast-radius/scope/rollout + PR-sizing/slicing); L4 — simplicity / over-engineering (`warning`-default). Each agent receives the plan doc path, an `instructions` field narrowing its dispatch to its single lens, and an `executor_profile` field identifying the executor (e.g. `"executor = subagent-driven-development with file access + TDD"`); each lens reads the WHOLE plan. Model-pin each agent to Sonnet.
...
4. ONE **challenge + dedup convergence pass** over the collected findings (see the paragraph after the example dispatch), which ends in a **tiered adversarial verify** over the surviving `error` tail (per `dispatching-parallel-agents` → `skills/dispatching-parallel-agents/references/verify-protocol.md`, `plan-review` profile): a batched triage pass, then escalate ONLY the contested tail to a minority-veto 3-voter consensus — not per-finding voting on every finding.
5. Cite this front-door: dispatching-parallel-agents §"Dispatching in prose" Shape A.

**Example dispatch (repeat for each of the 4 lenses):**
```
Agent {
...
After all 4 lenses return, run the **challenge + dedup convergence pass** — the four lenses run in isolation and cannot debate peer-to-peer, so the synthesizer plays referee: (a) **dedup** — collapse overlapping findings across lenses into a single candidate (two lenses flagging the same root issue become one); (b) **one shared severity bar** — re-classify each candidate `error` against the strict blocker definition (would fail the plan, or cause an incorrect or irreversible outcome) and demote any non-blocker to `warning` (a lens raising an `error` does not make it a blocker — the shared bar decides); (c) **disclosed-risk rule** — a risk the plan itself discloses and accepts is NOT re-raised as `error` (note it as informational; do not gate on it). Then run the **tiered adversarial verify** over the SURVIVING `error` tail (`skills/dispatching-parallel-agents/references/verify-protocol.md`, `plan-review` profile): (1) one batched triage labels each `error` `supported`/`uncertain`/`unsupported`; drop `unsupported` (false positives); (2) the Tier-1 escalation set (`uncertain`, or `disagree`-flagged `error`s — `disagree` = two lens agents cited conflicting evidence on the same item) goes through a clustered re-check that re-reads each finding's cited plan section and keeps/drops it; (3) the survivors form the contested tail, which escalates to a 3-voter minority-veto consensus — three architect voters with distinct framings each independently try to REFUTE the item; it survives as a genuine blocker only if ≥2 of 3 fail to refute, else it is dropped and logged `contested` (the false-negative trail). Merge the surviving + contested sets, then synthesize the SINGLE `APPROVED` / `NEEDS REVISION` verdict (NEEDS REVISION iff ≥1 surviving `error`).

**Iteration rules (loop until blockers clear; 3-round pause):**

| `error` finding type | Handling |
|---|---|
| User-judgment question (not researchable) | Surface to the user verbatim. Do not resolve with assumptions. Update plan after user responds. Re-invoke the 4-lens panel. |
| Design flaw resolvable from available context | Resolve from context. Update plan. Re-invoke the 4-lens panel. |

Each re-invocation dispatches the 4-lens panel as a fresh pass — no memory of prior rounds. `warning` and `Strengths` findings are informational only. Loop until a round yields ZERO surviving blockers — that is the APPROVED condition. **3-round pause:** if blockers remain after round 3, do not silently hard-stop and do not attempt a round unbidden — PAUSE and surface the remaining blockers to the user as a checkpoint, offering **continue** (run another round) / **intervene** (user resolves a blocker directly) / **accept** (proceed despite remaining findings). The user decides; the loop pauses rather than terminating on its own. ExitPlanMode only after an APPROVED verdict (or an explicit user `accept`).

**APPROVED ≠ exhaustive coverage.** The architect panel is a sampling review — non-deterministic and non-exhaustive. An `APPROVED` verdict means no `error`-severity findings survived the convergence pass in this pass, not that the plan is provably correct. A later round can catch a bug an earlier round missed. The proof of correctness is execution and tests; do not gate solely on architect approval.
