...
6. **Configure pre-commit hook** — set `scripts/run-tests.sh` (generated by `/e2e-init`) as the pre-commit script. The global hook entry point is installed by `setup.sh`; per-repo configuration is handled by `/e2e-init`.
