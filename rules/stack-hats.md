...
- A repo opts in via `project.json`: `"stacks": ["python", "react"]`. The `stacks` array may be hand-edited, or auto-populated by the `project-setup` skill's **Phase 4 — Tooling Setup**, which detects the repo's stack(s) and proposes the array (propose-then-confirm; never silently written).
